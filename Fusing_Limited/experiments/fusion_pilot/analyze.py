    ...
    for _, r in df.iterrows():
        for omega in [1, 2, 4, 8, 16]:
            cost = omega * r['T_r'] + 1 * r['T_d']
                rows.append(dict(case_name=r.get('case', r.get('case_name')), regime=r['regime'], delta=r['delta'], seed=r.get('seed_id', r.get('seed')), omega_R=omega, omega_D=1, cost=cost, T_r=r['T_r'], T_d=r['T_d'], T_total=r['T_total'], success=r.get('success', False)))
    ...
    for case in cases:
        for delta in deltas:
            ...
            for seed in seeds:
                ...
                for omega in [1, 2, 4, 8, 16]:
                    try:
                        r_R = rows_s[rows_s['regime'] == 'reward_only'].iloc[0]
                        r_D = rows_s[rows_s['regime'] == 'duel_only'].iloc[0]
